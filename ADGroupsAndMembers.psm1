...
# Luokka käyttäjä- ja ryhmätiedoille
            ...
            foreach($Ryhmä in $Ryhmät)
{
            ...
            foreach($Jäsen in $Jäsenet)
    {
            ...
            if($Jäsen.objectClass -eq "user")
        {
            ...
            Write-Output $DokumentoidutJäsenet
        }
   }
  ...
  # Lohko, joka suoritetaan työfunktion jälkeen
    END 
    {
        ...
        $Timer.Stop()
        $ElapsedTime = $Timer.Elapsed.TotalSeconds
        Write-Warning "<Groups and Their Members documented in $Elapsedtime seconds> $ElapsedTime seconds"
     } END PÄÄTTYY
